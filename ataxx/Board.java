...
class Board {
    ...
    void clear() {
        ...
        _undoSquares = new Stack<>();
        _undoPieces = new Stack<>();
        _totalOpen = (EXTENDED_SIDE - 4) * (EXTENDED_SIDE - 4);
        ...
        for (int i = 0; i < _board.length; i++) {
            if (i == index('a', '7')
                ...
                _board[i] = RED;
                    ...
                    || i == index('a', '1')) {
                _board[i] = BLUE;
            } else if (i < EXTENDED_SIDE * 2 || i >= EXTENDED_SIDE * 9) {
                _board[i] = BLOCKED;
    ...
    void makeMove(Move move) {
        ...
        if (!canMove(_whoseMove) && !canMove(opponent)) {
            getWinner();
        }
        if (move.isPass()) {
            pass();
        ...
        for (int sq = move.toIndex() - 1; sq <= move.toIndex() + 1; sq++) {
            for (int i = sq - EXTENDED_SIDE; i <= sq + EXTENDED_SIDE; i += 11) {
                if (_board[i] == opponent) {
                    addUndo(i);
                    unrecordedSet(i, _whoseMove);
                    incrPieces(_whoseMove, 1);
    ...
    void undo() {
        ...
        _whoseMove = _whoseMove.opposite();
        Move lastMove = _allMoves.get(numMoves() - 1);

        PieceColor deletedColor = _board[lastMove.toIndex()];
        int former = _undoSquares.pop();

        while (former != 0) {
            _board[former] = _undoPieces.pop();
            incrPieces(_board[former], 1);
            incrPieces(_board[former].opposite(), -1);
            former = _undoSquares.pop();
        }

        _board[lastMove.toIndex()] = EMPTY;
        _board[lastMove.fromIndex()] = deletedColor;

        if (lastMove.isExtend()) {
            incrPieces(deletedColor, -1);
        } else {
            _numJumps--;
        }

        _allMoves.remove(numMoves() - 1);
        _winner = null;
        announce();
    }
    ...
    boolean legalBlock(char c, char r) {
        return get(index(c, r)) == EMPTY && numMoves() == 0;
    }
